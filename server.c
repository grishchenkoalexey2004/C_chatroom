...
//if user is an admin, gives him admin rights by changing flag in structure with information about the client
int give_admin_rights(int client_num){
	if (verify_password(clients[client_num].last_msg) == 1){
		clients[client_num].status = ADMIN_STATUS;
		clients[client_num].sends_passw = 0;
		return 1;
	}
	return 0;
}
...
void send_auth_fail(int client_num){
	int put_star = 0;
	send_to_client(client_num,"ADMIN AUTHENTICATION FAILED!",put_star);
	return;
}
...
void nick_cmd(int caller_id){
	...
	if (strcmp(first_arg,clients[caller_id].name)!=0){
		send_to_client(caller_id,"OLD NICKNAME INCORRECT!",0);
		return;
	}

	strcpy(clients[caller_id].name,second_arg);
	send_to_client(caller_id,"NICKNAME CHANGED!",0);
	...
	for (int client_index=0;client_index<MAX_CLIENT_NUM;client_index++){
		if (is_connected(client_index) && (FD_ISSET(clients[client_index].sock_fd, sockets))){
			...
			if (clients[client_index].sends_passw){
				is_admin = give_admin_rights(client_index);
				if (is_admin){
					printf("***%s IS NOW AN ADMIN!",clients[client_index].name);
